  ...
  for (int channel = 0; channel < AUDIO_OUTPUTS; channel++) {
    ...
    if (outBlock[channel]) {
      for (int i = 0; i < AUDIO_BLOCK_SAMPLES; i++) {
        ...
        /*if(channel == 0) currentSample = currentSample; //distortionL.tick(currentSample);
        else if(channel == 1) currentSample = currentSample; //distortionR.tick(currentSample);*/
